...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
    ...
    if data is not None:
        ...
        data_reshaped = None
    if seg is not None:
        seg_reshaped = resample_data_or_seg(seg, new_shape, True, axis, order_seg, do_separate_z,
                                            order_z=order_z_seg)
    else:
        seg_reshaped = None
...
class GenericPreprocessor(object):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        list_of_cropped_npz_files = subfiles(folder=input_folder_with_cropped_npz, join=True, prefix=None, suffix=".npz", sort=True)
